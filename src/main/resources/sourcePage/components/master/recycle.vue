<template>
  <div class="recycle">
    <h3 class="recycle-title">回收站</h3>
    <div class="blog-list clearfix">
      <ul>
        <li v-for="(item,index) in recycles" :key="item.articleId">
          <h3>
          ...
          <div class="blog-handle">
            ...
            <span class="publish check">
              <a href="#" @click.prevent="recovery(item.articleId, index)">发布</a>
            </span>
            <span class="delete">
...
export default {
  data () {
    return {
      ...
      // 标识在第几页
      pageIndex: 1,
      pageUrl: '/article/condition/select?userId=' + window.sessionStorage.getItem('userId') + '&articleStatus=2'
    }
  },
  components: {
  ...
  methods: {
      ...
      this.axios.get(url)
        .then(res => {
          if (res.data.success) {
            ...
            this.recycles = res.data.data.content
          } else {
    ...
    // 恢复草稿
    recovery (id, index) {
      this.remove(id, index, 'article/recover', '恢复')
    },
    // 移除草稿
    del (id, index) {
